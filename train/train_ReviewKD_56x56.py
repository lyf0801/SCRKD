...
def main(scale):
    ...
    if not os.path.exists('./data/PSPNet_ReviewKD_' + scale + 'to56_EORSSD'):
        os.makedirs('./data/PSPNet_ReviewKD_' + scale + 'to56_EORSSD')
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >= 70:       
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = 'PSPNet_ReviewKD_' + scale + 'to56', student_size= 56, dataset = 'EORSSD', txt_name='PSPNet_ReviewKD_' + scale + 'to56_EORSSD.txt', output_dir = './data/output', cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), './data/PSPNet_ReviewKD_' + scale + 'to56_EORSSD/epoch_{}_{}.pth'.format(epoch,fm))

if __name__ == '__main__':
    teacher_scales = ['224', '112']
