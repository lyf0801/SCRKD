...
def main(dataset):
    ...
    if not os.path.exists("./data/PSPNet_SCRKD5_448to112_"+ dataset):
        os.makedirs("./data/PSPNet_SCRKD5_448to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss_sod, loss_multi_view_similarity, loss_multi_scale_feature_loss, loss_prediction_distillation = net(inputs_112x112, inputs_448x448, labels_112x112, labels_448x448)
            ##########loss#############
        ...
        if epoch >= 70:
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_SCRKD5_448to112", student_size= 112, dataset = dataset, txt_name="PSPNet_SCRKD5_448to112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/PSPNet_SCRKD5_448to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
