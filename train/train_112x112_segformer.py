...
def main(dataset):
    ...
    if not os.path.exists("./data/SegFormer_112x112_"+ dataset):
        os.makedirs("./data/SegFormer_112x112_"+ dataset)
    ...
    for epoch in range(0, args['epoch']): 
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "SegFormer_112x112", dataset = dataset, txt_name="SegFormer_112x112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/SegFormer_112x112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
