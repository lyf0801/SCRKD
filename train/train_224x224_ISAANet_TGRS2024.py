...
def main(dataset):
    ...
    if not os.path.exists("./data/ISAANet_224x224_"+ dataset):
        os.makedirs("./data/ISAANet_224x224_"+ dataset)
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >= 40:   
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "ISAANet_224x224", dataset = dataset, txt_name="ISAANet_224x224_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/ISAANet_224x224_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
