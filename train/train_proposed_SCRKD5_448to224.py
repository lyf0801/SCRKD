...
from model.pspnet_proposed_SCRKD52 import PSPNet_SCRKD as Net  #speficily for 224 student with 448 teacher, remove 448 cosine similarity
...
def main(dataset):
    ...
    if not os.path.exists("./data/PSPNet_SCRKD5_448to224_"+ dataset):
        os.makedirs("./data/PSPNet_SCRKD5_448to224_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        for i, data in enumerate(train_loader):
            ...
            labels_448x448 = data['label_448x448']
            ...
            labels_448x448 = labels_448x448.type(torch.FloatTensor).cuda()
            optimizer.zero_grad()
            loss_sod, loss_multi_view_similarity, loss_multi_scale_feature_loss, loss_prediction_distillation = net(inputs_224x224, inputs_448x448, labels_224x224, labels_448x448)
            ##########loss#############
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_SCRKD5_448to224", student_size= 224, dataset = dataset, txt_name="PSPNet_SCRKD5_448to224_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/PSPNet_SCRKD5_448to224_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
