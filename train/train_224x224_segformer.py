...
def main(dataset):
    ...
    if not os.path.exists("./data/SegFormer_224x224_"+ dataset):
        os.makedirs("./data/SegFormer_224x224_"+ dataset)
    
    #############################ResNet pretrained###########################
    #res18[2,2,2,2],res34[3,4,6,3],res50[3,4,6,3],res101[3,4,23,3],res152[3,8,36,3]
    model = Net(img_size=(224, 224))
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "SegFormer_224x224", dataset = dataset, txt_name="SegFormer_224x224_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/SegFormer_224x224_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
