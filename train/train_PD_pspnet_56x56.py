...
def main(scale):
    ...
    if scale == '112':
        ...
        model = Net(scale_factor=16)
        model.teacher.load_state_dict(torch.load('./data/PSPNet_448x448_EORSSD/epoch_81_0.8730648159980774.pth'))
    
    if not os.path.exists('./data/PSPNet_PD_' + scale + 'to56_EORSSD'):
        os.makedirs('./data/PSPNet_PD_' + scale + 'to56_EORSSD')
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss_sod, loss_ISR, loss_KD = net(inputs_56x56, teacher_inputs, labels_56x56)
            ##########loss#############
        ...
        if epoch >= 70:       
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = 'PSPNet_PD_' + scale + 'to56', student_size= 56, dataset = 'EORSSD', txt_name='PSPNet_PD_' + scale + 'to56_EORSSD.txt', output_dir = './data/output', cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), './data/PSPNet_PD_' + scale + 'to56_EORSSD/epoch_{}_{}.pth'.format(epoch,fm))
