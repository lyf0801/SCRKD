...
args = {
    'iter_num': 0,
    'epoch': 100,
    'train_batch_size': 4, #RuntimeError: CUDA out of memory. 
    'last_iter': 0,
    'snapshot': ''
}
...
def main(dataset):
    ...
    if not os.path.exists("./data/SegFormer_448x448_"+ dataset):
        os.makedirs("./data/SegFormer_448x448_"+ dataset)
    
    #############################ResNet pretrained###########################
    #res18[2,2,2,2],res34[3,4,6,3],res50[3,4,6,3],res101[3,4,23,3],res152[3,8,36,3]
    model = Net(img_size=(448, 448))
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "SegFormer_448x448", dataset = dataset, txt_name="SegFormer_448x448_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/SegFormer_448x448_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
