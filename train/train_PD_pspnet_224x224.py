...
def main(dataset, scale):
    ...
    if not os.path.exists('./data/PSPNet_PD_' + scale + 'to224_'+dataset):
        os.makedirs('./data/PSPNet_PD_' + scale + 'to224_'+dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss_sod, loss_ISR, loss_KD = net(inputs_224x224, teacher_inputs, labels_224x224)
            ##########loss#############
        ...
        if epoch >= 70:       
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = 'PSPNet_PD_' + scale + 'to224', student_size= 224, dataset = dataset, txt_name='PSPNet_PD_' + scale + 'to224_'+dataset+'.txt', output_dir = './data/output', cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), './data/PSPNet_PD_' + scale + 'to224_'+dataset+'/epoch_{}_{}.pth'.format(epoch,fm))
