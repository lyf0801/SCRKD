...
def main(dataset):
    ...
    if not os.path.exists("./data/SOLNet_56x56_"+ dataset):
        os.makedirs("./data/SOLNet_56x56_"+ dataset)
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >= 80:   
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "SOLNet_56x56", dataset = dataset, txt_name="SOLNet_56x56_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/SOLNet_56x56_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
