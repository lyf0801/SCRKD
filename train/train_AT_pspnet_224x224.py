...
def main(dataset):
    ...
    if not os.path.exists("./data/PSPNet_AT_448to224_"+ dataset):
        os.makedirs("./data/PSPNet_AT_448to224_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_AT_448to224", student_size= 224, dataset = dataset, txt_name="PSPNet_AT_448to224_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/PSPNet_AT_448to224_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
