...
def main(dataset):
    ...
    if not os.path.exists("./data/PSPNet_ReviewKD_448to112_"+ dataset):
        os.makedirs("./data/PSPNet_ReviewKD_448to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch == 0 or epoch >= 70: 
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_ReviewKD_448to112", student_size= 112, dataset = dataset, txt_name="PSPNet_ReviewKD_448to112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/PSPNet_ReviewKD_448to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
