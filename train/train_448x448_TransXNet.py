...
args = {
    'iter_num': 0,
    'epoch': 100,
    'train_batch_size': 8, #torch.cuda.OutOfMemoryError: CUDA out of memory
    'last_iter': 0,
    'snapshot': ''
}
...
def main(dataset):
    ...
    train_dataset = ORSI_SOD_Dataset(root = data_path,  img_size = 448,  mode = 'train', aug = True)
    train_loader = DataLoader(train_dataset, batch_size = args['train_batch_size'], shuffle = True, num_workers = 8)
    test_set = ORSI_SOD_Dataset(root = data_path,  img_size = 448, mode = "test", aug = False) 
    test_loader = DataLoader(test_set, batch_size = 1, shuffle = True, num_workers = 1)
    args['iter_num'] = args['epoch'] * len(train_loader)

    if not os.path.exists("./data/TransXNet_448x448_"+ dataset):
        os.makedirs("./data/TransXNet_448x448_"+ dataset)
    
    #############################ResNet pretrained###########################
    #res18[2,2,2,2],res34[3,4,6,3],res50[3,4,6,3],res101[3,4,23,3],res152[3,8,36,3]
    if dataset == "EORSSD":
        model = Net(image_size=448, in_stride=2)
    else: #ORSSD and ORS_4199
        model = Net(image_size=448, in_stride=4)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >=70:
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "TransXNet_448x448", dataset = dataset, txt_name="TransXNet_448x448_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/TransXNet_448x448_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
