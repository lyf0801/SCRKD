...
def main(dataset):
    ...
    if not os.path.exists("./data/ISAANet_56x56_"+ dataset):
        os.makedirs("./data/ISAANet_56x56_"+ dataset)
    ...
    optimizer = torch.optim.Adam(params=filter(lambda x: x.requires_grad, model.parameters()), lr=8e-4, betas=(0.5, 0.999), eps=1e-08)
    #########################################################################
    max_fm = 0
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >= 40:   
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "ISAANet_56x56", dataset = dataset, txt_name="ISAANet_56x56_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/ISAANet_56x56_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
