...
from model.segformer_STONet import SegFormer_STONet as Net
...
def main(dataset):
    ...
    if not os.path.exists("./data/SegFormer_STONet_224to112_"+ dataset):
        os.makedirs("./data/SegFormer_STONet_224to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        for i, data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            loss_sod, loss_distillation = net(inputs_112x112, inputs_224x224, labels_112x112, labels_224x224)
            ##########loss#############
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "SegFormer_STONet_224to112", student_size= 112, dataset = dataset, txt_name="SegFormer_STONet_224to112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/SegFormer_STONet_224to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
