...
def main(dataset):
    ...
    test_set = ORSI_SOD_Dataset(root = data_path, mode = "test", aug = False) #test_set 直接448
    test_loader = DataLoader(test_set, batch_size = 1, shuffle = True, num_workers = 1)
    args['iter_num'] = args['epoch'] * len(train_loader)

    if not os.path.exists("./data/PSPNet_448x448_"+ dataset):
        os.makedirs("./data/PSPNet_448x448_"+ dataset)
    ...
    for epoch in range(0, args['epoch']): 
        ...
        #eval
        thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_448x448", dataset = dataset, txt_name="PSPNet_448x448_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
        logg, fm = thread.run()
        print(logg)
        if fm > max_fm:
            max_fm = fm
            torch.save(net.state_dict(), "./data/PSPNet_448x448_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
