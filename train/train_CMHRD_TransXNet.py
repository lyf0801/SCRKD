...
def main(dataset):
    ...
    model = Net(img_size=112, s_gap=1)
    if dataset == 'ORSSD':
    ...
    if not os.path.exists("./data/TransXNet_CMHRD_224to112_"+ dataset):
        os.makedirs("./data/TransXNet_CMHRD_224to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch == 0:
            ...
            log_text = f'\n TransXNet_CMHRD Epoch training time: {epoch_time:.2f}s'
            # 使用标准open写入（追加模式）
        ...
        if epoch >= 70:    
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "TransXNet_CMHRD_224to112", student_size= 112, dataset = dataset, txt_name="TransXNet_CMHRD_224to112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/TransXNet_CMHRD_224to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
