...
from model.TransXNet_proposed_SCRKD5_parameter_analysis import TransXNet_SCRKD5 as Net
...
def main(dataset, a, b, c):
    ...
    model = Net(img_size=112, a=a, b=b, c=c)
    if dataset == 'ORSSD':
    ...
    if not os.path.exists("./data/TransXNet_SCRKD5_224to112_"+ dataset):
        os.makedirs("./data/TransXNet_SCRKD5_224to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >= 70:    
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "TransXNet_SCRKD5_224to112", student_size= 112, dataset = dataset, txt_name="TransXNet_SCRKD5_224to112_{}_a={}_b={}_c={}.txt".format(dataset, a, b, c), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                torch.save(net.state_dict(), "./data/TransXNet_SCRKD5_224to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
