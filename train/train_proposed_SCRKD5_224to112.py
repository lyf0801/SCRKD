...
from model.pspnet_proposed_SCRKD5 import PSPNet_SCRKD as Net
...
def main(dataset):
    ...
    model = Net()
    if dataset == 'ORSSD':
        model.teacher.load_state_dict(torch.load("./data/PSPNet_224x224_ORSSD/epoch_98_0.8890412449836731.pth"))
    elif dataset == "EORSSD":
        model.teacher.load_state_dict(torch.load("./data/PSPNet_224x224_EORSSD/epoch_85_0.8455814123153687.pth"))
    elif dataset == "ORS_4199":
        model.teacher.load_state_dict(torch.load("./data/PSPNet_224x224_ORS_4199/epoch_96_0.8553153872489929.pth"))
    
    if not os.path.exists("./data/PSPNet_SCRKD5_224to112_"+ dataset):
        os.makedirs("./data/PSPNet_SCRKD5_224to112_"+ dataset)
    ##############################Optim setting###############################
    ...
    for epoch in range(0, args['epoch']): 
        ...
        if epoch >=70:
            #eval
            thread = Eval_thread(epoch = epoch, model = net.eval(), loader = test_loader, method = "PSPNet_SCRKD5_224to112", student_size= 112, dataset = dataset, txt_name="PSPNet_SCRKD5_224to112_{}.txt".format(dataset), output_dir = "./data/output", cuda=True)
            logg, fm = thread.run()
            print(logg)
            if fm > max_fm:
                max_fm = fm
                torch.save(net.state_dict(), "./data/PSPNet_SCRKD5_224to112_"+ dataset +'/epoch_{}_{}.pth'.format(epoch,fm))
