...
class Eval_thread():
    def __init__(self, epoch, model, loader, dataset, method, txt_name, output_dir, student_size, cuda):
        self.model = model 
        model.eval() 
    ...
    def Eval_mae(self):
        ...
        with torch.no_grad():
            for data in tqdm(self.loader): 
                if self.student_size == 56:
                    img_lr = data['img_56x56']
                    img_hr = data['img_112x112']
                    gt = data['label_56x56']
                elif self.student_size == 112:
                    img_lr = data['img_112x112']
                    img_hr = data['img_224x224']
                    gt = data['label_112x112']
                elif self.student_size == 224:
                    img_lr = data['img_224x224']
                    img_hr = data['img_448x448']
                    gt = data['label_224x224']
                img_lr, img_hr, gt = img_lr.cuda(), img_hr.cuda(), gt.cuda() 
                if self.cuda:
                    pred = self.model(img_lr, img_hr, gt)  
                    pred = pred.cuda()
                    gt = gt.cuda()
                else:
                    pred = self.model(img_lr, img_hr, gt)
