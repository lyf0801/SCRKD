...
def compute_multi_view_self_similarity(tensor):
    ...
    channel_similarity = torch.zeros(B, C, C)
    for b in range(B):
    ...
    height_similarity = torch.zeros(B, H, H)
    for b in range(B):
    ...
    width_similarity = torch.zeros(B, W, W)
    for b in range(B):
...
class PSPNet_SCRKD(nn.Module):
    ...
    def forward(self, x_lr, x_hr=None, labels_lr=None, labels_hr=None):
        ...
        if self.training:
            """
            compute multi-view correlation distillation loss
            """
            loss_multi_view_similarity = self.similarity_criterion(student_channel_similarity1, teacher_channel_similarity1) + self.similarity_criterion(student_channel_similarity2, teacher_channel_similarity2) + self.similarity_criterion(student_channel_similarity3, teacher_channel_similarity3) + self.similarity_criterion(student_channel_similarity4, teacher_channel_similarity4) + self.similarity_criterion(student_channel_similarity5, teacher_channel_similarity5) +\
                                         self.similarity_criterion(student_height_similarity1, teacher_height_similarity1) + self.similarity_criterion(student_height_similarity2, teacher_height_similarity2) + self.similarity_criterion(student_height_similarity3, teacher_height_similarity3) + self.similarity_criterion(student_height_similarity4, teacher_height_similarity4) + self.similarity_criterion(student_height_similarity5, teacher_height_similarity5) +\
            ...
            """
            loss_multi_scale_feature_loss = hcl(compute_similarity_weighted_fusion(merge_student_feature1, teacher_out2), teacher_out2) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature2, teacher_out3), teacher_out3) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature3, teacher_out4), teacher_out4) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature4, teacher_out5), teacher_out5) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature5, F.interpolate(teacher_out5, size = out5.size()[2:], mode='bilinear', align_corners=True)), F.interpolate(teacher_out5, size = out5.size()[2:], mode='bilinear', align_corners=True))
            """
