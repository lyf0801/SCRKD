#speficily for 224 student with 448 teacher, remove 448 cosine similarity
#!/usr/bin/python3
#coding=utf-8
import torch
...
class PSPNet_SCRKD(nn.Module):
    ...
    def forward(self, x_lr, x_hr=None, labels_lr=None, labels_hr=None):
        ...
        """
        merge_student_feature1 = resize_and_combine_features([out13, out23, out33, out43, out53], out13.size()[2:])    
        merge_student_feature2 = resize_and_combine_features([out13, out23, out33, out43, out53], out23.size()[2:])   
        merge_student_feature3 = resize_and_combine_features([out13, out23, out33, out43, out53], out33.size()[2:])   
        merge_student_feature4 = resize_and_combine_features([out13, out23, out33, out43, out53], out43.size()[2:])   
        merge_student_feature5 = resize_and_combine_features([out13, out23, out33, out43, out53], out53.size()[2:])   
        B, C, H, W, _ = merge_student_feature1.size()
        merge_student_feature1 = self.adaptive1(merge_student_feature1.permute(0, 4, 1, 2, 3).contiguous().view(B * 5, C, H, W)).view(B, 5, C, H, W).permute(0, 2, 3, 4, 1)
        B, C, H, W, _ = merge_student_feature2.size()
        merge_student_feature2 = self.adaptive2(merge_student_feature2.permute(0, 4, 1, 2, 3).contiguous().view(B * 5, C, H, W)).view(B, 5, C, H, W).permute(0, 2, 3, 4, 1)
        B, C, H, W, _ = merge_student_feature3.size()
        merge_student_feature3 = self.adaptive3(merge_student_feature3.permute(0, 4, 1, 2, 3).contiguous().view(B * 5, C, H, W)).view(B, 5, C, H, W).permute(0, 2, 3, 4, 1)
        B, C, H, W, _ = merge_student_feature4.size()
        merge_student_feature4 = self.adaptive4(merge_student_feature4.permute(0, 4, 1, 2, 3).contiguous().view(B * 5, C, H, W)).view(B, 5, C, H, W).permute(0, 2, 3, 4, 1)
        B, C, H, W, _ = merge_student_feature5.size()
        merge_student_feature5 = self.adaptive5(merge_student_feature5.permute(0, 4, 1, 2, 3).contiguous().view(B * 5, C, H, W)).view(B, 5, C, H, W).permute(0, 2, 3, 4, 1)
        ...
        student_smap1 = self.SOD_head1(out1)
        ...
        student_smap2 = self.SOD_head2(out2)
        ...
        student_smap3 = self.SOD_head3(out3)
        ...
        student_smap4 = self.SOD_head4(out4)
        att5 = self.att_conv5(torch.cat((out51, out52, out53), dim=1))
        n,_,h,w = out51.shape
        out5 = self.fusion_conv5((out51 * att5[:,0].view(n,1,h,w) + out52 * att5[:,1].view(n,1,h,w) + out53 * att5[:,2].view(n,1,h,w)))
        student_smap5 = self.SOD_head5(out5)
        ### interpolate
        ...
        if self.training:
            ...
            """
            #student_channel_similarity1, student_height_similarity1, student_width_similarity1 = compute_multi_view_self_similarity(F.interpolate(out12, size = teacher_out1.size()[2:], mode='bilinear', align_corners=True))
            student_channel_similarity2, student_height_similarity2, student_width_similarity2 = compute_multi_view_self_similarity(F.interpolate(out22, size = teacher_out2.size()[2:], mode='bilinear', align_corners=True))
            student_channel_similarity3, student_height_similarity3, student_width_similarity3 = compute_multi_view_self_similarity(F.interpolate(out32, size = teacher_out3.size()[2:], mode='bilinear', align_corners=True))
            student_channel_similarity4, student_height_similarity4, student_width_similarity4 = compute_multi_view_self_similarity(F.interpolate(out42, size = teacher_out4.size()[2:], mode='bilinear', align_corners=True))
            student_channel_similarity5, student_height_similarity5, student_width_similarity5 = compute_multi_view_self_similarity(F.interpolate(out52, size = teacher_out5.size()[2:], mode='bilinear', align_corners=True))
                
            #teacher_channel_similarity1, teacher_height_similarity1, teacher_width_similarity1 = compute_multi_view_self_similarity(teacher_out1)
            teacher_channel_similarity2, teacher_height_similarity2, teacher_width_similarity2 = compute_multi_view_self_similarity(teacher_out2)
            teacher_channel_similarity3, teacher_height_similarity3, teacher_width_similarity3 = compute_multi_view_self_similarity(teacher_out3)
            teacher_channel_similarity4, teacher_height_similarity4, teacher_width_similarity4 = compute_multi_view_self_similarity(teacher_out4)
            teacher_channel_similarity5, teacher_height_similarity5, teacher_width_similarity5 = compute_multi_view_self_similarity(teacher_out5)

            loss_multi_view_similarity =  self.similarity_criterion(student_channel_similarity2, teacher_channel_similarity2) + self.similarity_criterion(student_channel_similarity3, teacher_channel_similarity3) + self.similarity_criterion(student_channel_similarity4, teacher_channel_similarity4) + self.similarity_criterion(student_channel_similarity5, teacher_channel_similarity5) +\
                                         self.similarity_criterion(student_height_similarity2, teacher_height_similarity2) + self.similarity_criterion(student_height_similarity3, teacher_height_similarity3) + self.similarity_criterion(student_height_similarity4, teacher_height_similarity4) + self.similarity_criterion(student_height_similarity5, teacher_height_similarity5) +\
                                          self.similarity_criterion(student_width_similarity2, teacher_width_similarity2) + self.similarity_criterion(student_width_similarity3, teacher_width_similarity3) + self.similarity_criterion(student_width_similarity4, teacher_width_similarity4) + self.similarity_criterion(student_width_similarity5, teacher_width_similarity5)
            """
            speficily for 224 student with 448 teacher, remove 448 cosine similarity
            """
            #self.similarity_criterion(student_channel_similarity1, teacher_channel_similarity1) +
            #self.similarity_criterion(student_height_similarity1, teacher_height_similarity1) + 
            #self.similarity_criterion(student_width_similarity1, teacher_width_similarity1) +
            """
            compute multiscale feature distillation loss
            """
            loss_multi_scale_feature_loss = hcl(compute_similarity_weighted_fusion(merge_student_feature1, F.adaptive_avg_pool2d(teacher_out2, output_size=out1.size()[2:])), F.adaptive_avg_pool2d(teacher_out2, output_size=out1.size()[2:])) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature2, F.adaptive_avg_pool2d(teacher_out3, output_size=out2.size()[2:])), F.adaptive_avg_pool2d(teacher_out3, output_size=out2.size()[2:])) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature3, F.adaptive_avg_pool2d(teacher_out4, output_size=out3.size()[2:])), F.adaptive_avg_pool2d(teacher_out4, output_size=out3.size()[2:])) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature4, F.adaptive_avg_pool2d(teacher_out5, output_size=out4.size()[2:])), F.adaptive_avg_pool2d(teacher_out5, output_size=out4.size()[2:])) +\
                                            hcl(compute_similarity_weighted_fusion(merge_student_feature5, F.adaptive_avg_pool2d(teacher_out5, output_size=out5.size()[2:])), F.adaptive_avg_pool2d(teacher_out5, output_size=out5.size()[2:]))
            ...
            loss_prediction_distillation = CriterionKD(F.interpolate(student_smap1, size = teacher_smap1.size()[2:], mode='bilinear', align_corners=True), teacher_smap1) +\
                                           CriterionKD(F.interpolate(student_smap2, size = teacher_smap2.size()[2:], mode='bilinear', align_corners=True), teacher_smap2) +\
                                           CriterionKD(F.interpolate(student_smap3, size = teacher_smap3.size()[2:], mode='bilinear', align_corners=True), teacher_smap3) +\
                                           CriterionKD(F.interpolate(student_smap4, size = teacher_smap4.size()[2:], mode='bilinear', align_corners=True), teacher_smap4) +\
                                           CriterionKD(F.interpolate(student_smap5, size = teacher_smap5.size()[2:], mode='bilinear', align_corners=True), teacher_smap5) 
            ...
            return loss_sod, loss_multi_view_similarity, loss_multi_scale_feature_loss, loss_prediction_distillation * 10
