...
class SegFormer_STONet(nn.Module):
    def __init__(self, img_size):
        super(SegFormer_STONet, self).__init__()
    ...
    def forward(self, x_lr, x_hr=None, labels_lr=None, labels_hr=None):
        ...
        if self.training:
            ...
            loss_FAKD = self.frequency_kd2(teacher_out4, teacher_out3, teacher_out2, teacher_out1, student_out4, student_out3, student_out2, teacher_out1)
            loss_DDKD = self.feature_kd_loss(teacher_out1, student_out1)

            loss_SRKD = self.KLDLoss(student_smap1, teacher_smap1, labels_hr, 4) 
            loss_DICE = dice_loss(student_smap1, teacher_smap1) 
            ...
            loss_distillation = (loss_FAKD + loss_DDKD + loss_SRKD + loss_DICE) * 0.1 #+ loss0
