...
class TransXNet_PD(nn.Module):
    def __init__(self, img_size:int, scale_factor:int):
        """
        scale_factor = teacher_size / student_size * 2
        """
        super(TransXNet_PD, self).__init__()
    ...
    def forward(self, x_lr, x_hr=None, labels_lr=None):
        ...
        if self.training:
            ...
            loss_ISR = self.isr_criterion(image_sr, x_hr) * 0.1 #balanced weights for TransXNet_PD
            # Response KD
